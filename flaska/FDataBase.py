...
class FDataBase:
    ...
    def register_employee(self, login, phone_number,position_id, hash_pwd):
        try:
            ...
            if res[0] > 0:
                ...
                return False
            self.__cur.execute("INSERT INTO employee(login, phone_number, position_id, password) VALUES(%s, %s, %s, %s)", (login, phone_number, position_id, hash_pwd))
            self.__db.commit()
        except:
    ...
    def getUser(self, user_id):
        try:
            self.__cur.execute(f"SELECT * FROM employee JOIN positions on positions.position_id = employee.position_id WHERE employee_id = {user_id} LIMIT 1")
            res = self.__cur.fetchone()
            if not res:
                print("Пользователь не найден")
                return False 
